...
for (const dpr of [1, 2]) {
  ...
  await p.evaluate(async () => {
    ...
    for (let y = 0; y < h; y += window.innerHeight * 0.6) { window.scrollTo(0, y); await new Promise(r => setTimeout(r, 260)); }
  ...
  const r = await p.evaluate((dpr) => {
    return [...document.querySelectorAll('img')].map((im) => {
      ...
      const archivo = decodeURIComponent(u.searchParams.get('url') || u.pathname).split('/').pop();
      const necesita = Math.round(rect.width * dpr);
      return {
        archivo, servido: w ? +w : null, q: q ? +q : null,
        cssW: Math.round(rect.width), necesita,
  ...
  for (const x of r) {
    ...
    console.log(`${(x.archivo||'').padEnd(34)} css ${String(x.cssW).padStart(4)} · necesita ${String(x.necesita).padStart(4)} · servido ${String(x.servido).padStart(4)} (q${x.q}) · ratio ${x.ratio}${flag}`);
  }
